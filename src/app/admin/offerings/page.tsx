...
const typeLabels: Record<"all" | OfferingType, string> = {
  all: "Todos", class: "Talleres", workshop: "Workshops", experience: "Experiencias", gift_card: "Gift Cards",
};
  ...
  return (
    <AdminShell>
      <div className="section-head">
        <div><p className="auth-kicker">Contenido comercial</p><h2>Offerings</h2></div>
        <Link className="primary-btn inline" href="/admin/offerings/new">Crear contenido</Link>
      </div>
      <div className="filters">
        <div className="filter-group">
          {(["all", "class", "workshop", "experience", "gift_card"] as const).map((item) => (
            <Link key={item} className={item === type ? "chip active" : "chip"} href={`/admin/offerings?type=${item}&status=${status}`}>{typeLabels[item]}</Link>
          ))}
        </div>
        <div className="filter-group">
          {(["all", "draft", "published", "archived"] as const).map((item) => (
            <Link key={item} className={item === status ? "chip active" : "chip"} href={`/admin/offerings?type=${type}&status=${item}`}>{statusLabels[item]}</Link>
          ))}
        </div>
      </div>
